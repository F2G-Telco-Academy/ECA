...
import scat.writers
import scat.parsers
from scat.writers.observability_writer import ObservabilityWriter
...
def scat_main():
    ...
    parser.add_argument('-L', '--layer', help='Specify the layers to see as GSMTAP packets (comma separated).\nAvailable layers: {}, Default: "ip,mac,rlc,pdcp,rrc,nas"'.format(', '.join(valid_layers)), type=str, default='ip,mac,rlc,pdcp,rrc,nas')
    ...
    if args.pcap_file == None:
        base_writer = scat.writers.SocketWriter(GSMTAP_IP, GSMTAP_PORT, IP_OVER_UDP_PORT)
    else:
        base_writer = scat.writers.DualWriter(args.pcap_file, GSMTAP_IP, GSMTAP_PORT, IP_OVER_UDP_PORT)
    
    # Wrap with full observability (comprehensive telecom parsing enabled)
    writer = ObservabilityWriter(base_writer)
    ...
    if args.serial or args.usb:
        try:
            ...
            # Ignore USB timeout errors on stop_diag (device might be busy)
            logger.warning(f'Failed to stop diag (ignoring): {e}')
        
        try:
            current_parser.init_diag()
        except Exception as e:
            # Ignore USB timeout errors on init_diag (device might be busy)
            logger.warning(f'Failed to init diag (ignoring): {e}')
        
        try:
            current_parser.prepare_diag()
        except Exception as e:
            # Ignore USB timeout errors on prepare_diag (device might be busy)
            logger.warning(f'Failed to prepare diag (ignoring): {e}')
