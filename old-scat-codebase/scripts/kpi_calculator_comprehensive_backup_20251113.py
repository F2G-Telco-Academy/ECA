...
class SimpleKPICalculator:
    ...
    def count_packets(self, pcap_file, filter_expr):
        ...
        cmd = ['tshark', '-r', pcap_file, '-Y', filter_expr, '-T', 'fields', '-e', 'frame.number']
        try:
    ...
    def get_packet_details(self, pcap_file, filter_expr):
        ...
        cmd = ['tshark', '-r', pcap_file, '-Y', filter_expr, '-T', 'fields', 
    ...
    def analyze(self, pcap_file):
        ...
        self.kpis = {
            ...
            'lte_erab_success': (lte_erab_setup / max(lte_rrc_setup, 1)) * 100,
        ...
        total_procedures = sum([lte_rrc_req, lte_attach_req, lte_pdn_req, lte_service_req, wcdma_rrc_req, call_setup])
...
LTE (4G) KPIs:
  ...
  E-RAB Accessibility: {self.kpis.get('lte_erab_success', 0):.2f}% [Target: >98%]
    Setups: {self.kpis.get('lte_erab_setup', 0)} (after {self.kpis.get('lte_rrc_setup', 0)} RRC setups)
...
class MetricsHandler(BaseHTTPRequestHandler):
    ...
    def get_detailed_statistics_static():
        ...
        return {
            ...
            'lte': {
                ...
                'erab': {
                    'success_rate': round(calc.kpis.get('lte_erab_success', 0), 2),
    ...
    def get_prometheus_metrics(self):
        ...
        lines.append(f"scat_lte_rrc_success_rate {calc.kpis.get('lte_rrc_success', 0)}")
        
        lines.append("# TYPE scat_lte_attach_success_rate gauge")
        lines.append(f"scat_lte_attach_success_rate {calc.kpis.get('lte_attach_success', 0)}")
        
        lines.append("# TYPE scat_lte_tau_success_rate gauge")
        lines.append(f"scat_lte_tau_success_rate {calc.kpis.get('lte_tau_success', 0)}")
        
        lines.append("# TYPE scat_lte_erab_success_rate gauge")
        lines.append(f"scat_lte_erab_success_rate {calc.kpis.get('lte_erab_success', 0)}")
        
        lines.append("# TYPE scat_lte_pdn_success_rate gauge")
        lines.append(f"scat_lte_pdn_success_rate {calc.kpis.get('lte_pdn_success', 0)}")
        
        lines.append("# TYPE scat_lte_service_success_rate gauge")
        lines.append(f"scat_lte_service_success_rate {calc.kpis.get('lte_service_success', 0)}")
        
        # WCDMA KPIs
        lines.append("# TYPE scat_wcdma_rrc_success_rate gauge")
        lines.append(f"scat_wcdma_rrc_success_rate {calc.kpis.get('wcdma_rrc_success', 0)}")
        
        lines.append("# TYPE scat_wcdma_rab_success_rate gauge")
        lines.append(f"scat_wcdma_rab_success_rate {calc.kpis.get('wcdma_rab_success', 0)}")
        
        lines.append("# TYPE scat_wcdma_handover_success_rate gauge")
        lines.append(f"scat_wcdma_handover_success_rate {calc.kpis.get('wcdma_ho_success', 0)}")
        
        # Call Control KPIs
        lines.append("# TYPE scat_call_success_rate gauge")
        lines.append(f"scat_call_success_rate {calc.kpis.get('call_success', 0)}")
        
        lines.append("# TYPE scat_lte_call_drop_rate gauge")
        lines.append(f"scat_lte_call_drop_rate {calc.kpis.get('call_drop_rate', 0)}")
        
        # GSM KPIs
        lines.append("# TYPE scat_gsm_rach_attempts counter")
        lines.append(f"scat_gsm_rach_attempts {calc.kpis.get('rach_attempts', 0)}")
        
        # Placeholder metrics for not-yet-implemented KPIs (will show 0)
        lines.append("# TYPE scat_lte_handover_success_rate gauge")
        lines.append("scat_lte_handover_success_rate 0")
        
        lines.append("# TYPE scat_lte_bearer_modify_success_rate gauge")
        lines.append("scat_lte_bearer_modify_success_rate 0")
        
        lines.append("# TYPE scat_lte_rsrp_avg gauge")
        lines.append("scat_lte_rsrp_avg 0")
        
        lines.append("# TYPE scat_lte_rsrq_avg gauge")
        lines.append("scat_lte_rsrq_avg 0")
        
        lines.append("# TYPE scat_wcdma_rscp_avg gauge")
        lines.append("scat_wcdma_rscp_avg 0")
        
        lines.append("# TYPE scat_wcdma_ecno_avg gauge")
        lines.append("scat_wcdma_ecno_avg 0")
